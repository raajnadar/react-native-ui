...
export function blendColor(base: string, overlay: string, overlayAlpha: number): string {
  ...
  const r = Math.round((1 - boundedAlpha) * baseChannels.r + boundedAlpha * overlayChannels.r);
  const g = Math.round((1 - boundedAlpha) * baseChannels.g + boundedAlpha * overlayChannels.g);
  const b = Math.round((1 - boundedAlpha) * baseChannels.b + boundedAlpha * overlayChannels.b);
